...
/*
 This code controls the interactions with the Type Puzzle View.
 It makes a costumized keyboard to pop up for spelling the puzzle words.
...
- (void)viewDidLoad
{
    ...
    [self setupTouchEventGesture]; // Disabled by Javad - 14-7-2016
...
{
    // loads the background image of the current selected puzzle.
	...
	[self slideOutKeyboard]; //slides out the soft key board
    
	NSString *title = [_object.title uppercaseString]; //takes the title of the current puzzle in uppercases
	
	for (int i = 0; i < title.length; i++) { // reads the characters in the current title and creates enabled buttons for the pop up keyboard for these characters.
	...
	for (int i = 0; i < [_keys count]; i++) {                                                       // loads the rest of the keys for keyboard as blank
		UIButton *button = [_keys objectAtIndex:i];		
		if (button.enabled == NO)
			button.titleLabel.text = @"";
	}
	
	_currentLetterPosition = -1; // initiates current letter position
	[self performSelector:@selector(playObjectTitleSound) withObject:nil afterDelay:0.5];           //Plays the title sound for the selected puzzle
...
- (void)playObjectTitleSound // A method for playing the title sound for each selected puzzle
{
...
- (void)touchesBegan:(NSSet *)touches withEvent:(UIEvent *)event
{
    ...
    // added by Javad 14-11-2016 to return the distance to the correct key
...
{
    _currentLetterPosition++; // relocated from 487 - date: 11-7-2016
	NSString *title = [_object.title uppercaseString];
	...
	//_currentLetterPosition++; // commented out - date 11-7-2016
	button = [self buttonFromASCIICode:[title characterAtIndex:_currentLetterPosition]];
	button.alpha = 1.0;
    
    
    //Added by Javad to log the highleghted key event - 6-12-2016
    [[EventLogger sharedLogger] logEvent:LogEventCodeKeyHighlighted eventInfo:@{@"letter":[[_object.title uppercaseString] substringWithRange:NSMakeRange(_currentLetterPosition, 1)],@"key x": [NSString stringWithFormat: @"%.2f", button.center.x], @"key y": [NSString stringWithFormat: @"%.2f", button.center.y  + [[UIScreen mainScreen] bounds].size.height - _keyboard.frame.size.height]}];
    // ----------------------------------------------------------
...
- (void)remindAnimation:(UIGestureRecognizer *)recognizer {
    ...
    for ( UIView *piece in _pieces) {
        ...
        if(_currentLetterPosition>-1){
            ...
            // added by Javad 25-10-2016 to add coordinates to reminder event
            NSString *title = [_object.title uppercaseString];
            UIButton *expectedButton = [self buttonFromASCIICode:[title characterAtIndex:_currentLetterPosition]];
            // --------------------------------------------------------------
            
            // added by Javad 25-10-2016 to add key letter and key coordinates to reminder event
            [[EventLogger sharedLogger] logEvent:LogEventCodeTypeReminder eventInfo:@{@"key": [[_object.title uppercaseString] substringWithRange:NSMakeRange(_currentLetterPosition, 1)],@"key x": [NSString stringWithFormat: @"%.2f", expectedButton.center.x], @"key y": [NSString stringWithFormat: @"%.2f", expectedButton.center.y  + [[UIScreen mainScreen] bounds].size.height - _keyboard.frame.size.height]}];
            
                //NSLog(@"%c", [[_object.title uppercaseString] characterAtIndex:_currentLetterPosition]);
                
                //[_object.title characterAtIndex:_currentLetterPosition]);
            break;
            //}
        }
    }
...
-(void)handlePanGestureRecognized:(UIGestureRecognizer *)recognizer{
    ...
    if (recognizer.state == UIGestureRecognizerStateEnded && _currentLetterPosition>-1) {
        ...
        if(touchLocation.x && touchLocation.y){ // Added by Javad 22-8-2016
            ...
            NSLog(@"XXXXXXXXXXXXXXXXXXXXXXXX"); // Added by Javad 22-8-2016
        }
    }
    }
}
