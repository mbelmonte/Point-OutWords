...
- (void)handleTapGesture:(UITapGestureRecognizer *)gesture
{
    ...
    [[EventLogger sharedLogger] logEvent:LogEventCodeTouchEnded eventInfo:@{@"X": [NSString stringWithFormat:@"%+.1f", initialTouchPoint.x], @"Y": [NSString stringWithFormat:@"%+.1f", initialTouchPoint.y]}];
...
- (void)handlePanGesture:(UIPanGestureRecognizer *)gesture
{
	if (gesture.state == UIGestureRecognizerStateBegan) {
		//CGPoint initialTouchPoint = [gesture locationOfTouch:0 inView:self.view]; // disabled by Javad 19-8-2016
        /* reference: http://stackoverflow.com/questions/27081999/pan-gesture-recognizer-location-of-touch-crashes-once-the-pan-gesture-is-releas */
        CGPoint initialTouchPoint = [gesture locationInView:self.view]; // added by Javad 19-8-2016

        // check distance from puzzle pieces -- find the nearest and return the distance (added by Javad 14-11-2016)
        CGFloat distance = 1024;
        CGFloat newDistance = 1024;
        PuzzlePieceView *nearestPiece = nil;
        for (PuzzlePieceView *piece in _pieces) {
            CGPoint point = [self.view convertPoint:initialTouchPoint toView:piece];
        ...
        // -----------------------------------------------------------------------------
        [[EventLogger sharedLogger] logEvent:LogEventCodeTouchBegan eventInfo:@{@"X": [NSString stringWithFormat:@"%+.1f", initialTouchPoint.x], @"Y": [NSString stringWithFormat:@"%+.1f", initialTouchPoint.y], @"nearest piece distance": [NSString stringWithFormat:@"%+.1f", distance]}]; // modified to return distance to the nearest puzzle piece
        
        NSLog(@"Touch Began"); // line added by Javad - 18-10-2016
        ...
        //touchcount++;
		if (_draggedPiece != nil) {
			//CGPoint newLocation = [gesture locationOfTouch:0 inView:self.view]; // disabled by Javad 19-8-2016
            CGPoint newLocation = [gesture locationInView:self.view]; // added by Javad 19-8-2016 - as referenced above
        ...
        [[EventLogger sharedLogger] logEvent:LogEventCodeTouchEnded eventInfo:@{@"X": [NSString stringWithFormat:@"%+.1f", touchLocation.x], @"Y": [NSString stringWithFormat:@"%+.1f", touchLocation.y]}];
        NSLog(@"Touch Ended");
